...
dijkstra <-function(graph,init.node){
  ...
  weight_matrix <- matrix(NA,no_of_nodes ,no_of_nodes)
  for (i in 1:length(graph$v1)) {
    weight_matrix[graph$v1[i],graph$v2[i]]<-graph$w[i]
  }
  ...
  while (length(unvisited_set) > 1) {
    for (i in unvisited_set) {
      #cat("distance",current_node,",",i,",",weight_matrix[current_node,i],"\n")
      if(i!=current_node && !is.na(weight_matrix[current_node,i])){
        new_distance <-weight_matrix[current_node,i]+tentative_distance[[current_node]]
